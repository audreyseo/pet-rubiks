...
//	}]
angular.module('myApp', ['ngCookies']).controller('dictator', function($scope) {
	
	})
	.controller('myController', ['$scope', '$cookieStore',  'orderByFilter', 'filterFilter', function($scope, cookieStore, orderBy, filterFilter) {
		$scope.cases = [
...
//				console.log("hiding cookie safe");
		...
		$scope.timeElapsed = function() {
			var date = new Date();
			if (starttime) {
				$scope.endTime = date.setDate(date.getDate()) - starttime;
			} else {
				$scope.endTime = date.setDate() - date.setDate();
			}
			...
			if (event.keyCode) {
				...
				if (event.keyCode === 32) {
					if (starttime) {
						...
						var date = new Date();
						starttime = date.setDate(date.getDate());
						$scope.endTime = date.setDate(date.getDate()) - starttime;
					}
				}
			}
		...
		$scope.filterMyCases = function() {
			$scope.filteredCases = $scope.cases;
			for (var idx = 0; idx < $scope.filteredCases.length; idx++) {
				console.log(idx + ": " + $scope.filteredCases[idx].code);
				if ($scope.hiddenRows[$scope.filteredCases[idx].code] == 1) {
					delete $scope.filteredCases[idx];
					idx --;
				}
			}
		};
		...
		$scope.hideAway = function() {
			$("#editRow").hide($scope.animationOpts);
			console.log("hiding known");
			$(".known").hide($scope.animationOpts);
		...
		$scope.showAgain = function() {
			$("#editRow").show($scope.animationOpts);
//			console.loge("Showing known");
			$(".known").show($scope.animationOpts);
			$scope.showingEditRow = true;
			$scope.showingEditCol = true;
		...
		$scope.initialize = function() {
			...
			$scope.showing = {editRow:true, editCol:false};;
...
//			$scope.showAgain();
		...
		$scope.createOLLArray = function() {
			var numbers = [8, 2, 4, 2, 2, 6, 4, 4, 6, 2, 2, 4, 4, 6];
			var codePrefixes = ["OCLL", "E", "P", "W", "S", "L", "F", "A", "LB", "T", "C", "I", "K", "O"];
			var codes = [];
			for (var i = 0; i < codePrefixes.length; i++) {
				for (var j = 1; j <= numbers[i]; j++) {
					codes.push(codePrefixes[i] + j);
				}
			}
			return codes;
		};
		
		$scope.customComparator = function(object1, object2) {
			var codes = $scope.createOLLArray();
			var index1 = codes.indexOf(object1.value);
			var index2 = codes.indexOf(object2.value);
			console.log("one: " + index1 + " two: " + index2);
			var result = (index1 > index2) ? -1 : ((index1 === index2) ? 0 : 1);
			return result;
		}
		
		$scope.sortBy = function(name) {
			$scope.setSort(name);
			console.log($scope.comparisons.custom);
			if ($scope.comparisons.custom && name == 'code') {
				console.log('custom sorting...');
				$scope.filteredCases = orderBy($scope.filteredCases, $scope.column, $scope.reverse, $scope.customComparator);
			} else {
				$scope.filteredCases = orderBy($scope.filteredCases, $scope.column, $scope.reverse);
			}
		};
...
//			$scope.filteredCases = $scope.cases;
//			for (var idx = 0; idx < $scope.filteredCases.length; idx++) {
//				if (newValue[$scope.cases[idx].code]) {
//					if (newValue[$scope.cases[idx].code] == 1) {
//					delete $scope.filteredCases[idx];
//					idx--;
//					}
//				}
...
//		});
		...
		$scope.editTable = function() {
			...
			if ($scope.showing.editRow) {  //editingRow.is(":visible")) {
//				console.log("Hiding");
//				$("#editRow").hide();
				$scope.showing.editRow = false;
				$(".known").hide();
