...
app.controller('ContentController', ['$scope', 'cases', 'hiddenRowsY', 'cookieStrings', 'cookieSavedData', '$cookies',  'orderByFilter', 'filterFilter', function($scope, cases, hiddenRows, cookieString, cookieData, $cookies, orderBy, filterFilter) {
		cookieData.initialize();	
		...
		$scope.flashCardsData = function() {
			...
			$scope.cards = {maxNumber: 0, options: []};
			try {
				$scope.cards = $cookies.getObject($scope.cookieString.cards);
//				console.log((typeof $scope.cards.maxNumber));
//				console.log((typeof $scope.cards.options));
				console.log("Object == typeof $scope.cards: " + ("object" === (typeof $scope.cards)));
				console.log("Array == [1,2,3]: " + ("object" === (typeof [1,2,3])));
//				console.log("Why???: " + (typeof $scope.cards));
				
				if (!angular.isNumber($scope.cards.maxNumber)) {
//					$scope.cards = {maxNumber: 0, options: []};
					$scope.cards.maxNumber = parseInt($scope.cards.maxNumber);
					$cookies.putObject($scope.cookieString.cards, $scope.cards);
				}
			} catch(e) {
				$scope.cards = {maxNumber: 0, options: []};
