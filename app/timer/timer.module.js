...
//  	console.log(angular.toJson($scope.records['index']));
...
//  	    link.setAttribute("href", encodedUri);
  	...
  	$scope.csvContent += "\n" + "index,time,millis,timeStamp"
		
		for (var i = 0; i < $scope.records.index.length; i++) {
			var infoArray = ["index", "time", "millis", "timeStamp"];
...
//				dataString += "," + $scope.records[infoArray[j]][i];
    ...
    var link = document.createElement('a');
		link.setAttribute('href', $scope.encodedUri);
  	link.setAttribute('download', "data.csv");
  	link.setAttribute('id', "downloadLink");
  	$(link).html("Download");
  	link = $(link);
  	$("#saveButtonDiv").after(link);
  }
